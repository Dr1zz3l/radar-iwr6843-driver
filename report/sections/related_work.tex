...
similarly fuse radar velocity with an \ac{imu} in continuous time, but
attack the same radar elevation drift we target
(\autoref{sec:baselines}) with an uncertainty-aware ground-plane model
their denser 4D cloud supports,
rather than outlier rejection; that anchor assumes a persistently
visible floor, whereas our tumbling regime sees it only intermittently,
so we close the gap with a \ac{ransac} front-end needing no ground return
(a floor-plane factor gated on the frames that do observe it is future
work, \autoref{sec:conclusion}).  Single-chip radar-inertial estimation has itself
...
12-channel single-chip array) on a quadrotor in aggressive racing and
