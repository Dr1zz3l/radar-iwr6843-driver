...
alone.  A magnetometer resolves this and is standard on flight platforms;
here \ac{mocap} yaw serves that role.  Because that
...
the real-sensor gap two ways.  The dominant magnetometer error is a hard-iron
offset: a constant vector in the \emph{body} frame (often $5$--$15\degree$
uncalibrated).  In near-level flight its tilt-compensated horizontal
projection is essentially time-constant, hence a heading \emph{gauge} that
our start-anchored alignment absorbs, leaving drift, velocity, and
orientation \ac{rmse} unchanged to first order however large the offset.
Only the \emph{time-varying} residual costs: perturbing the prior with a
$2\degree$ slowly-varying drift plus $1.5\degree$ noise (three seeds) degrades
live orientation by up to ${\sim}1.5\degree$ on slow racing ($2.1$--$3.5\degree$
from $1.97\degree$; the estimate tracks the prior) and ${\sim}0.4\degree$ on
fast racing, and a $\lambda_\psi$ sweep shows this is the sensor's floor, not
over-trust ($\lambda_\psi{=}10$ near-optimal).  A throttle-correlated real
magnetometer, validated against ground truth, is future work.  The
...
The fixed-lag smoother initially performed poorly on
...
Three fixes account for the working result.
...
noise.
The trust allocation is fully automated (the
dynamics-adaptive law); the two rescue terms stay manual because $b$
awaits a better measurement model (per-chirp timestamps) and the tether
awaits enough flights to validate an observability-driven adaptive rule.
...
This same over-dense parameterization is the source of the batch bistability noted
above: the final cost \emph{anti-correlates} with accuracy (the
underconstrained problem overfits the wrong global orientation), so
single-run batch comparisons at this density are not trustworthy.
...
array.  A joint calibration experiment \emph{disproves} this: (i)~with pitch
...
inclinometer; \autoref{sec:results}).
...
eigenvalues $\sim$$5.5{\times}10^{14}$ vs.\ position $\sim$$10^4$) blocks any
single scalar, and eigenvalue clipping and Cauchy robustification also fail
(Cauchy down-weights the bag that needs the prior most).
...
\ac{ransac} default brings the same metric to 0.382\,m
...
\subsection{Adaptive Knot Density}
\label{sec:negative}
...
rotate to it (open-loop gyro dead-reckoning at 7.8\degree\ already beats the
9.2\degree\ this fusion reaches before the rate-adaptive gyro trust of
\autoref{sec:backflips}).  The lever is measurement weighting, not knot
density, \emph{disproving} the adaptive-knot-density hypothesis and obviating
the non-uniform-basis extension.
