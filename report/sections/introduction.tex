...
position estimate drifts with distance, by construction rather than by
...
\begin{itemize}
  ...
  \item A \emph{consistency-analyzed} fixed-lag smoother: a factor-set
        ...
        complementary to \ac{fej}, eliminating prior-weight tuning, with a
        real-time operating point per regime at minimal accuracy cost
        (0.3--0.5\,s stride on a laptop).
  \item One \emph{weighting law} from hover to
        \SI{10}{\radian\per\second} (with per-regime spline grids and two
        tumbling rescue terms), the velocity--orientation trade a measured
        Pareto curve, validated on \emph{held-out racing flights}
        (tumbling awaits the repeated flips of \autoref{sec:stat_scope}).
