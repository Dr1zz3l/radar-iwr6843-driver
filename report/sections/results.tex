...
We use the full-trajectory batch solve as an offline full-batch
reference; offline it reaches
\SI{0.20}{\metre}\,/\,\SI{1.1}{\degree} on slow racing and
\SI{0.74}{\metre}\,/\,\SI{2.4}{\degree} on fast (the dense racing
...
\multirow{2}{*}{Fast racing}
  & Settled & 0.25 & 2.34 & 0.382 & 0.85 & \multirow{2}{*}{0.35} \\
  & \textbf{Live edge} & \textbf{0.32} & \textbf{2.90} & \textbf{0.469} & \textbf{1.04} & \\
\midrule
...
\multicolumn{7}{@{}p{\columnwidth}@{}}{\scriptsize
  ...
  inversion); the \textbf{bold} live-edge rows are the deployment-relevant figures throughout.
...
Live-edge orientation degrades 0.4--0.6\degree\ relative to settled;
...
\ac{rpe} converges to $1.0$--$2.1\%$ beyond 20\,m on racing (from
...
reach it per regime costs almost nothing.
Fast is real-time at the 0.3\,s stride with a 2\,s window, slow and
backflips at a 0.5\,s stride (1.5\,s and 2\,s windows);
each drops per-window solve below its stride
(0.28/0.40/0.43\,s for fast/slow/backflips).
Slow racing and
...
($1.04\to1.15\%$ drift).  Solve times carry $\sim$20\% variance,
...
\begin{figure*}[t]
  ...
  \caption{Trajectory overlays for all three flights (columns): \ac{mocap}
    ...
    projections. Within each column both projections share a common $Y$ range
    and metric scale (equal aspect, so loops keep their true shape); the scale
    differs between columns, so compare shapes rather than absolute sizes across
    columns. Start~$\blacksquare$.  Larger fast-racing deviations
...
\begin{figure*}[t]
  ...
  \caption{SW live-edge (deployed) position, velocity, and orientation
    error vs.\ time for slow racing (top), fast racing (middle), and
    backflips (bottom).  Fast-racing position error accumulates
    monotonically with trajectory length; velocity and orientation errors
    spike at high-speed, high-dynamics sections; backflip errors peak
    \emph{during} the flips, where the radar measurement model degrades
    (\autoref{sec:backflips}).}
  \label{fig:error_time}
\end{figure*}

\subsection{Radar Contribution}
Radar is essential.  Disabling the radar factors collapses the
...
0.37\,m\,/\,2.81\degree\ live (vs.\ 0.47\,m\,/\,2.90\degree\ on the
...
older radar firmware (best-position firmware) with $12\times$ coarser Doppler quantization
...
Each flight condition is a single representative recording; the
...
A single-chip radar's small virtual aperture gives coarse
elevation resolution, and a minority of returns carry an
...
makes its contribution near-negligible), so they too estimate vertical from radar and
...
would confound it with our own parameter search.  Here (the reverse direction) we port their ekf-rio and
its multi-radar generalization
x-rio~\cite{doer2021xrio} (run single-radar) onto our data; ekf-yrio was the
comparator in the forward direction (our solver on their data) above.  On the slow-racing flight both ekf-rio
...
frame (their \ac{ransac}/$\sigma$-gates expect order-100 clouds, and ego-velocity
...
(measured in-flight gyro PSD ${\approx}2.1$ vs.\ their
${\approx}0.2\,\degree/\mathrm{s}/\sqrt{\mathrm{Hz}}$ default),
...
This reverse direction thus measures how their stock filters
\emph{couple} to an out-of-distribution sensor stack, not the relative quality
of the two estimators: any filter starves on ${\sim}13$-point clouds it was not
designed for, so we report it as a sensitivity characterization, not a
head-to-head result.
...
\parbox{\columnwidth}{\raggedright\scriptsize Live estimate, start-anchored
  alignment.  \textbf{(a)}~headline; heading aiding differs by design (see
  text).  \textbf{(b)}~decomposition: \emph{roll/pitch} is the
  heading-independent attitude \ac{rmse} (yaw excluded), so the orientation
  gap in~(a) is yaw, not attitude; \emph{our drift} splits position into
  horizontal/vertical (both sub-1.3\% under the default \ac{ransac} prefilter,
  \autoref{sec:models}); \emph{whole-traj.\ \ac{ate}} re-evaluates every system
  under EVO/KITTI Umeyama alignment (no scale) for cross-checking published
  figures.  Prefilter \emph{disabled} (Huber-only): vertical drift
  $12$--$15\%$, whole-traj.\ \ac{ate} $9.57/2.88/10.86/5.48$\,m (flights~1--4).\par}
\end{table}
...
included) and compute the \ac{nees} against the \emph{same} ground truth the
headline uses (Vicon-fed velocity, slerp attitude), after masking windows at
\ac{mocap} dropouts/glitches.  For a consistent estimator each 3-DoF block is
$\chi^2_3$ with \emph{mean}~3; we report the per-window mean and, as its
spread depends on the window count $N$, the 95\% interval for the
$N$-window mean (not the wide per-sample $\chi^2_3$ interval).
On the racing flights the covariance is \emph{calibrated} on fast-racing
orientation (\ac{nees} mean $3.2$, $N{=}20$, inside the $N$-window 95\%
interval $[2.0,4.2]$) and \emph{moderately conservative} on the other three
channels: slow orientation $1.6$ ($N{=}49$), slow velocity $2.0$, fast
velocity $1.0$, all below their lower bounds (slow $[2.4,3.7]$); i.e.\ the
weights over-report uncertainty, most strongly for fast-racing velocity
(${\sim}3\times$ in variance).  Conservative is the safe direction for a fusion
factor.  These are \emph{calibration checks}, not powered hypothesis tests:
consecutive windows are temporally correlated, so the expectation holds only
heuristically.  On backflips only 6 of 50 windows
survive the ground-truth mask (\autoref{sec:setup}), too few for a
consistency statement; there the covariance is overconfident (orientation
\ac{nees} mean~38, ${\sim}3.5\times$ in standard deviation; velocity~9.6),
reported as a caveat.  Deployment recipe: inflate the emitted covariance per
regime ($1\times$ gentle, $3\times$ aggressive), or calibrate online from
innovations.
...
\caption{Selected Ablation Results (batch solver, slow\_racing unless noted;
  SW rows use the default 0.3\,s stride; bold: default configuration)}
\label{tab:ablations}
\small
...
\quad 2.0\,s & 0.455 / 0.518 & 2.33 / 2.95 \\
\quad \textbf{3.0\,s (default)} & \textbf{0.382 / 0.469} & \textbf{2.34 / 2.90} \\
\midrule
\multicolumn{3}{l}{\textit{Marginalization (fast\_racing SW, settled)}\textsuperscript{\ddag}} \\
\quad None (boundary prior only) & 0.466 & 2.68 \\
\quad \textbf{Schur complement} & \textbf{0.382} & \textbf{2.34} \\
\bottomrule
\multicolumn{3}{@{}p{\columnwidth}@{}}{\scriptsize
  Batch rows (\ac{imu}-rate, lever-arm, ori-reg) isolate one
  ...
  fast 0.768$\to$0.74\,m, the \autoref{sec:results} batch figures) without
...
motivating the tight bias priors in the solver configuration.
...
Schur marginalization reduces fast-racing settled position by 18\%
(0.466\,m $\to$ 0.382\,m); for slow racing the optimal prior scale is
...
orientation at 2\,s and 3\,s (settled 2.33\degree\ vs 2.34\degree) with
3\,s modestly better on position (live drift 1.15 vs 1.04\%).  We keep
