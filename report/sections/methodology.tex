...
formulation~\cite{sommer2020efficient,usenko2020basalt}:
\begin{equation}
  \vR(t) = \vR_{k-3}
  ...
  \label{eq:ori_spline}
\end{equation}
where $k$ is the active knot index, the $\vR_i \in \SO$ are \emph{absolute}
rotation knots (the optimization variables; basalt
convention~\cite{usenko2020basalt}), $\Log(\vR_{j-1}^{-1}\vR_j)$ are their
relative increments, and $\tilde{B}_j(t)$ are cumulative cubic basis
functions.  Knot spacing is $\Delta t_o = 8$\,ms (slow racing; see
\autoref{tab:config}).
...
Of the radar extrinsics only the pitch $\delta_\text{pitch}$
is observable from Doppler (roll and yaw are fixed); the sliding window fixes
it at the measured $27.5\degree$, with batch self-calibration analyzed in
...
does not drive the choice.  This $\delta$ is deliberately large relative to
the $0.07$--$0.08$\,m/s ego-velocity residuals we ultimately reach
(\autoref{sec:baselines}): after the \ac{ransac} prefilter the inliers are
already clean, so $\delta$ caps only the rare survivor rather than shaping
the fit.

\textbf{\ac{ransac} ego-velocity prefilter (default front-end).}
A single-chip radar's small 12-channel virtual aperture has
coarse elevation resolution, and a minority of returns carry an elevation-correlated
...
unchanged.  On those sparse frames (most of the backflip regime) the
in-solver Huber loss is the sole radar outlier protection, so the two
stages are complementary.  It runs once at frame load, not per window, so
it does not enter the per-window timing budget (\autoref{sec:results}).
...
our own flights it improves aggressive-racing live position by $8\%$ ($0.51\to0.47$\,m
live).  It is enabled by default for every result in this paper.

\textbf{Dynamics-adaptive measurement weighting.}
Our central practical finding is that a single \emph{rate-adaptive trust
allocation} across the three measurement streams covers hover to
\SI{10}{\radian\per\second} with one configuration, each component derived
from a measured failure diagnosis (\autoref{sec:backflips}), not tuned ad hoc:
\begin{itemize}
\item \emph{Gyroscope: trust more when rotating fast.}
  We scale the per-sample gyro weight as
  $\lambda_g^{\mathrm{eff}} = \lambda_g\bigl(1 + (|\vz_g|/\omega_g)^p\bigr)$
  with $\lambda_g{=}4$, $\omega_g{=}4$\,rad/s, $p{=}4$, using the
  \emph{measured} rate $\vz_g$ (causal, no spline evaluation).  The quartic
...
\item \emph{Radar: trust less when rotating fast.}
  ...
  ($\approx$17\degree\ at 10\,rad/s).  We inflate the per-frame
  radar noise as
  $\sigma_{\mathrm{eff}}^2 = \sigma_0^2\bigl(1+(|\vom|/\omega_r)^2\bigr)$
  with
  $\omega_r{=}4$\,rad/s and $|\vom|$ from the warm-start spline at
  problem-build time; the down-weighting is continuous, with no
  hard-gate cliff.
\item \emph{Accelerometer: same form,} $\omega_a{=}8$\,rad/s:
  ...
  $(\omega_r,\omega_a){=}(4,8)$ is the knee of a measured velocity--orientation
  trade (\autoref{sec:backflips}).
...
backflips only; $u_{s,z}$ is the vertical component of the
sensor-frame bearing $\vu_s$).  \autoref{sec:backflips} shows why $b$ must be
interpreted as a flight-regime error proxy rather than a physical
elevation bias of the array.
...
\begin{equation}
  ...
  \label{eq:accel}
\end{equation}
with $\vz_a$ the measured specific force,
weighted $\lambda_a = 0.01$: this $400{:}1$ gyro/accel ratio
($\lambda_g{=}4$, below) is a deliberate design statement that leans
position and velocity on radar Doppler plus min-snap, since the
accelerometer carries thrust transients and frame vibration that
corrupt the $\vR$--$\ddot{\vp}$ coupling (the dynamics-adaptive law of
\autoref{sec:models} down-weights it further mid-flip).
...
(live orientation $2.0\to4.2\degree$) while a strong prior costs almost no
position.  On fast racing, the rich translational excitation already
holds yaw near $3.8\degree$ at $\lambda_\psi{=}0.6$, so raising it to
$10$ buys only $0.5\degree$ of yaw yet degrades live position
(drift $1.0\to1.3\%$) through the orientation--velocity coupling; we
therefore keep it light.  On backflips the anchor instead stabilizes the
whole attitude through the flip: dropping $\lambda_\psi$ to $0.6$ inflates
live orientation from $6.4\degree$ to $8.7\degree$ (all three axes
degrade), so it stays at $10$.  The per-regime weight is therefore not
monotone in dynamics: gentle (slow racing) and tumbling (backflips) flight
need the same strong prior at opposite rates while translation-rich fast
racing needs a weak one, so it cannot key off $|\vom|$.  It is thus the one
weight \emph{not} folded into the runtime-adaptive law above, set once per
flight type rather than auto-weighted; keying it instead off a
yaw-observability proxy such as translational excitation, and adapting it to
real magnetometer noise in place of the idealized \ac{mocap} heading, is
left to future work.
...
correct alias once at frame load (23.1\% of points aliased
in fast racing, 2.0\% in slow racing); the unwrapped Doppler is the
...
\begin{proposition}[Exact closed spline marginal at a fixed linearization]
\label{prop:marg}
At a fixed linearization point, let $\mathcal{M}$ be the knots leaving the
window and $\mathcal{B}$ the $N{-}1$ retained knots immediately
\emph{forward} of $\mathcal{M}$ plus the global 6-DoF \ac{imu} bias (the
dim-30 boundary of \autoref{sec:sw}).  Every order-$N$ spline factor has
support over $N$ consecutive knots, and an \ac{imu} factor also touches the
bias; hence any factor touching $\mathcal{M}$ has support
$\subseteq\mathcal{M}\cup\mathcal{B}$ with its bias dependence in
$\mathcal{B}$.  Factors reaching further back were consumed in the
\emph{previous} marginalization step and enter the current Schur complement
only through the incoming prior term, never re-added as fresh factors, so
nothing is double-counted.  Selecting exactly the factors touching
$\mathcal{M}$ (plus that prior) therefore forms a Schur complement that
conditions on no interior state and shares no factor with the next window:
at this linearization it is the \emph{exact} marginal.
\end{proposition}
...
thus yields a closed marginal.  Empirically it is also \emph{consistent}:
the prior is applied \textbf{unscaled} (scale $=1$) across all flight
regimes, and its Mahalanobis residual at the solution drops from
$10^{6}$--$10^{12}$ to $\mathcal{O}(1)$, i.e.\ it agrees with the incoming data.
