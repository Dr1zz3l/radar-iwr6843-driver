...
\textbf{\ac{imu} rate:} $\sim$993\,Hz (full rate, no downsampling).
...
differences of the estimated trajectory; the reference is the Vicon-fed
onboard state-estimate velocity, zero-phase low-pass filtered (4th-order
Butterworth, \SI{10}{\hertz}), not a raw finite difference of \ac{mocap}
position.
\emph{Position drift} (\%) is position \ac{rmse} normalised by the total
GT path length, the standard odometry metric for a
dead-reckoning system with no loop closure (per-segment \ac{rpe} in
\autoref{sec:sw_results}).
...
(SuiteSparse), up to 400 iterations (function-tolerance stop;
typically ${\sim}30$).
The analytic Jacobians combine the basalt cumulative-spline
Jacobian struct for the spline terms, closed-form expressions for the
\ac{imu} sensor terms, and SymForce-generated code~\cite{martiros2022symforce}
for the radar sensor term; Ceres automatic differentiation was
